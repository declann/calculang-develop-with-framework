...
export const upF = function (el, selected_formulae, formulae_not_inputs) { 
  ...
  for (let i of n) {
    ...
    if (first || !o.has(i)) { // misses already-removed elements => use dom?
      ...
      //if (b) {
        b.checked = (selected_formulae.value[i] != undefined /* need to capture 0s! */) ? selected_formulae.value[i] : false
      //} else {
      b.oninput = (e) => {debugger; selected_formulae.value = {...selected_formulae.value, [i]: (e.target.checked)}; selected_formulae.dispatchEvent(new CustomEvent("input"), {bubbles:true}) }
      //b.onchange = (e) => {cursor.value = {...cursor.value, [i]: (+e.target.value)}}
      s.appendChild(b)
      //s.appendChild()
      //s.appendChild(Inputs.range([0,1],{}))
      ...
      selected_formulae.value = {[i]: false, ...selected_formulae.value, }
